...
Ext.Osiris.RegisterListener("Equipped", 2, "after", function(item, character)
	if IsTagged(character,"WIZARD_BLADESINGING_bc52a699-cf55-4dad-9710-927e655910fd") == 1 and Osi.IsWeapon(item) == 1 then
		if Osi.IsTagged(item,"WPN_GREATAXE_02da79f5-6f13-4f90-9819-102e37693f48") == 1 or Osi.IsTagged(item,"WPN_MAUL_2503012b-9cc4-491c-a068-282f8cea8707") == 1 or Osi.IsTagged(item,"WPN_HALBERD_2c74855f-769a-43f5-b6db-48c4c47721ff") == 1 or Osi.IsTagged(item,"WPN_HEAVY_CROSSBOW_2cc23bb9-d777-4265-a34c-333528628b90") == 1 or Osi.IsTagged(item,"WPN_LONGBOW_557d335c-0780-4665-9802-709a7d202dba") == 1 or Osi.IsTagged(item,"WPN_LONGBOW_557d335c-0780-4665-9802-709a7d202dba") == 1 or Osi.IsTagged(item,"WPN_GLAIVE_7a15ea4f-cb00-4201-8e7f-024627e3d014") == 1 or Osi.IsTagged(item,"WPN_SHORTBOW_81197304-7116-4d7b-8ef4-207bbf636682") == 1 or Osi.IsTagged(item,"WPN_GREATCLUB_81197304-7116-4d7b-8ef4-207bbf636682") == 1 or Osi.IsTagged(item,"WPN_GREATSWORD_aec4ed1a-993b-491f-a2db-640bf11869c1") == 1 or Osi.IsTagged(item,"WPN_DART_c23ac9ef-5b47-4c2d-8ce5-7b60a8b34787") == 1 or Osi.IsTagged(item,"WPN_PIKE_ca1a548b-f409-4cad-af5a-dfdd5834c709") == 1 then
		Osi.ApplyStatus(character,"BLADESONG_WEAPON_MESSAGE",-1.0,1)
		elseif Osi.IsTagged(item,"WPN_GREATAXE_02da79f5-6f13-4f90-9819-102e37693f48") == 0 and Osi.IsTagged(item,"WPN_MAUL_2503012b-9cc4-491c-a068-282f8cea8707") == 0 and Osi.IsTagged(item,"WPN_HALBERD_2c74855f-769a-43f5-b6db-48c4c47721ff") == 0 and Osi.IsTagged(item,"WPN_HEAVY_CROSSBOW_2cc23bb9-d777-4265-a34c-333528628b90") == 0 and Osi.IsTagged(item,"WPN_LONGBOW_557d335c-0780-4665-9802-709a7d202dba") == 0 and Osi.IsTagged(item,"WPN_LONGBOW_557d335c-0780-4665-9802-709a7d202dba") == 0 and Osi.IsTagged(item,"WPN_GLAIVE_7a15ea4f-cb00-4201-8e7f-024627e3d014") == 0 and Osi.IsTagged(item,"WPN_SHORTBOW_81197304-7116-4d7b-8ef4-207bbf636682") == 0 and Osi.IsTagged(item,"WPN_GREATCLUB_81197304-7116-4d7b-8ef4-207bbf636682") == 0 and Osi.IsTagged(item,"WPN_GREATSWORD_aec4ed1a-993b-491f-a2db-640bf11869c1") == 0 and Osi.IsTagged(item,"WPN_DART_c23ac9ef-5b47-4c2d-8ce5-7b60a8b34787") == 0 and Osi.IsTagged(item,"WPN_PIKE_ca1a548b-f409-4cad-af5a-dfdd5834c709") == 0 then
...
EXP_tCommandUndead      = {};
tLimboUndead        = {};
local cmdud_status  = "COMMAND_UNDEAD";
local cmdud_trigger = "COMMAND_UNDEAD_TRIGGER";
local cmdud_failed  = "COMMAND_UNDEAD_FAILED";
local cmdud_timer   = "COMMAND_UNDEAD_TIMER";
local cmdud_drop    = "COMMAND_UNDEAD_DROP";

-- ////////////////////////////////////////////////////
-- Modvars stuff.
Ext.Vars.RegisterModVariable(ModuleUUID, "EXP_commandedundead", {});
Ext.Vars.RegisterModVariable(ModuleUUID, "EXP_limboundead", {});
function EXP_EXP_ObtainCommandedUndead()
...
Ext.Osiris.RegisterListener("LevelGameplayStarted", 2, "after", function(levelName, isEditorMode)
    local modvars   = EXP_EXP_ObtainCommandedUndead();
    EXP_tCommandUndead  = modvars.EXP_commandedundead;
    tLimboUndead    = modvars.EXP_limboundead;
end)
...
function EXP_UncommandUndead(casterID)
    ...
    ApplyStatus(ud, cmdud_drop, 6, 1, casterID);
    ...
    if tLimboUndead[ud] then
        tLimboUndead[ud] = nil;
            modvars.EXP_limboundead = tLimboUndead;
        end
    end

function EXP_CommandUndead(targetID, sourceID, is_int)
        if HasActiveStatus(targetID, cmdud_status) == 0 then
        ...
        if entity and entity.PartyFollower and entity.PartyFollower.Following then
            ...
            ApplyStatus(targetID, cmdud_status, -1, 1, sourceID);
    ...
    if is_int then
            ApplyStatus(targetID, cmdud_timer, 3600, 1, sourceID)
        end
end
...
Ext.Osiris.RegisterListener("StatusApplied",4,"after", function(targetID, status, sourceID, _)
    if status ~= cmdud_trigger and status ~= cmdud_failed then return; end
    ...
    or (EXP_tCommandUndead[source].Undead == targetID and status == cmdud_failed)) then
        EXP_UncommandUndead(source);
    end
    if status == cmdud_trigger then
        -- Target has advantage on 8 or higher int. Hourly re-roll on 12 or higher.
        local tgt_intelligence = GetAbility(targetID, "Intelligence");
        local is_int = tgt_intelligence > 11 and true or false;
        EXP_CommandUndead(targetID, source, is_int);
    end
end)
...
Ext.Osiris.RegisterListener("RespecCompleted",1,"after",function(charID)
    if next(EXP_tCommandUndead) == nil then return; end
    if EXP_tCommandUndead[charID] and HasActiveStatus(EXP_tCommandUndead[charID].Undead, cmdud_status) == 1 then
...
Ext.Osiris.RegisterListener("DB_PartyFollowers",1,"after",function(charID)
    if next(tLimboUndead) == nil then return; end
    if tLimboUndead[charID] and tLimboUndead[charID] ~= CharacterGetOwner(charID) then
...
Ext.Osiris.RegisterListener("CharacterJoinedParty",1,"after",function(charID)
    ...
    if EXP_tCommandUndead[charID] then
        for ud, src in pairs(tLimboUndead) do
            if EXP_tCommandUndead[charID].Undead == ud and (HasActiveStatus(ud, cmdud_status) == 1 or HasActiveStatus(ud, cmdud_timer) == 1) then
...
Ext.RegisterConsoleCommand("sailor_clearcommandundead", CommandUndead_Cleaner);
